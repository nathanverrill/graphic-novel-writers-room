"""Split the storycraft skill into per-role guides, so each agent reads only
the parts it needs (the whole skill is ~15k tokens per model call).

    python tools/split_skill.py

Rerun after editing skills/story_to_visual_translation_skill.md or the map below.
...
def main():
    ...
    for role, (focus, part_names, extra) in MAP.items():
        ...
        body = "\n".join("\n".join(c).strip() + "\n" for c in chunks)
        header = (f"<!-- generated by tools/split_skill.py from skills/{SOURCE.name}; edit the source and rerun -->\n\n"
                  f"# Storycraft: {focus}\n\n")
